import { getPosts, getProfile } from "@/sanity/actions"
          ...
          <div className="flex flex-col gap-5 px-5">
            <div className="flex flex-wrap ">
              ...
              <div className="flex flex-col lg:w-[40%] px-5">
                {
                  posts.slice(0, 3)?.map((post, i) => <article key={post._id} className="py-4 border-b dark:border-zinc-800 last:border-none">
                    <div className="flex flex-col sm:flex-row flex-nowrap -mx-3 sm:space-x-5">
                      <div className="w-full sm:w-auto">
                        <Link href={`/post/${post.slug}`}>
                          <Image src={post.mainImage} alt={post.title} width={168} height={168} className="object-cover rounded-lg w-full h-[235px] sm:h-auto" />
                        </Link>
                      </div>
                      <div className="sm:w-[60%] sm:ml-1">
                        <div className="mt-1 sm:mt-0 sm:mb-1">
                          <Link href={`?category=${post.category}`}>
                            <span className="bg-gradient-to-r from-purple-600 to-red-600 inline-block text-transparent bg-clip-text font-bold">{post.category}</span>
                          </Link>
                        </div>
            ...
            <div className="flex flex-col sm:flex-row items-start gap-2 sm:gap-8 sm:border-t dark:border-zinc-800">
              {
                posts.slice(2, 4).map((post, i) => <PostCard key={post._id} post={post} />)
              }
            </div>
          </div>
        ) : (
